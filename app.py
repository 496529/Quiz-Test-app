...
def _database_url():
    # Prefer Streamlit secrets (used on Streamlit Cloud); fall back to env var / .env locally.
...
@st.cache_resource
def get_pool():
    """A single pooled connection shared across reruns (Streamlit re-executes the script often)."""
    url = _database_url()
    if not url:
        st.error(
            "DATABASE_URL is not set. Add your Supabase connection string to "
            "`.streamlit/secrets.toml` (key: DATABASE_URL) or a local .env file."
        )
        st.stop()
...
def run(query, params=None, fetch=None, commit=False):
    """
    Small helper so call sites stay close to the original SQLite style.
    fetch: None | "one" | "all"
    """
    conn = get_conn()
    try:
...
def init_db():
    ...
    # Seed any categories/questions that don't already exist yet.
    # Safe to run on every startup — existing data (users, scores, past questions) is untouched.
    for cat_name in CATEGORIES:
        row = run("SELECT id FROM categories WHERE name = %s", (cat_name,), fetch="one")
        if row is None:
            row = run(
                "INSERT INTO categories (name) VALUES (%s) RETURNING id",
                (cat_name,), fetch="one", commit=True,
            )
        cat_id = row["id"]

        existing = run(
            "SELECT COUNT(*) AS n FROM questions WHERE category_id = %s",
            (cat_id,), fetch="one",
        )
        if existing["n"] == 0:
...
def register_user(username, password):
    try:
        row = run(
            ...
            (username, hash_pw(password)), fetch="one", commit=True,
        )
        return {"id": row["id"], "username": username}, None
...
def login_user(username, password):
    row = run(
        ...
        (username, hash_pw(password)), fetch="one",
    )
    if row:
...
def get_categories():
    return run("SELECT * FROM categories ORDER BY id", fetch="all")

def fetch_questions(category_id):
    return run(
        "SELECT * FROM questions WHERE category_id=%s ORDER BY RANDOM() LIMIT %s",
        (category_id, QUESTIONS_PER_QUIZ), fetch="all",
    )

def save_score(user_id, category_id, score, total):
    pct = (score / total) * 100
    run(
        "INSERT INTO scores (user_id,category_id,score,total,percentage) VALUES (%s,%s,%s,%s,%s)",
        (user_id, category_id, score, total, pct), commit=True,
    )
...
def get_leaderboard():
    return run("""
        SELECT u.username, ca.name AS category,
               ...
               to_char(s.played_at, 'DD Mon YYYY HH24:MI') AS played_on
        FROM scores s
